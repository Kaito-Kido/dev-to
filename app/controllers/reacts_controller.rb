class ReactsController < ApplicationController
  ...
  def create
    ...
    return unless @react.save
    return unless @post.user.id != current_user
  ...
  def find_post
    post_id = @reactable.instance_of?(::Post) ? @reactable.id : @reactable.post_id
