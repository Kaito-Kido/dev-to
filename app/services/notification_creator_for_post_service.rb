    ...
    when 'published'
      if @post.user.admin?
        User.all.each do |user|
          Notification.create(sender_id: @post.user.id, receiver_id: user.id, action: :post, content: "#{@post.user.name} has just posted an article", post_id: @post.id) if user != @post.user
        end
      else
