  ...
  def call
    if @current_user != @commentable.user
      ...
      if !is_post?(@commentable) && @commentable.user != @post.user
        Notification.create(sender_id: @current_user.id, receiver_id: @post.user.id, content: "#{@current_user.name} has commented to your post", action: :comment, post_id: @post.id, seen: false)
      end
    else
      Notification.create(sender_id: @current_user.id, receiver_id: @post.user.id, content: "#{@current_user.name} has replied to his comment on your post", action: :comment, post_id: @post.id, seen: false)
    end
  end
