class NotificationCreatorForCommentService < ApplicationService
  ...
  def call
    if @sender != @commentable.user
      Notification.create(sender_id: @sender.id, receiver_id: @commentable.user.id, content: "#{@sender.name} has #{post?(@commentable) ? 'commented' : 'replied'} to your #{post?(@commentable) ? 'post' : 'comment'}", action: :comment, post_id: @post.id)
      if !post?(@commentable) &&
